...
export class FilterFactory {
  ...
  public get(query: FilterFactoryQuery): AbstractFilter {
    if (this.isFieldFilter(query.key)) {
      ...
      const notOperator = query.key.includes(`${LookupDelimiter.LOOKUP_DELIMITER}${LookupFilter.NOT}`);
      const lookup = query.key.includes(LookupDelimiter.LOOKUP_DELIMITER)
        ? query.key.split(LookupDelimiter.LOOKUP_DELIMITER)[notOperator ? 2 : 1] as LookupFilter
        : LookupFilter.EXACT;
